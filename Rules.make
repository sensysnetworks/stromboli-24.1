...
%.s: %.cc
	$(CXX) $(CXXFLAGS) $(EXTRA_CXXFLAGS) $(CXXFLAGS_$@) -S $< -o $@

%.i: %.cc
	$(CXX) $(CXXFLAGS) $(EXTRA_CXXFLAGS) $(CXXFLAGS_$@) -E $< > $@

%.o: %.cc
	$(CXX) $(CXXFLAGS) $(EXTRA_CXXFLAGS) $(CXXFLAGS_$@) -c -o $@ $<
	@ ( \
...
#
# Additions to compile user-space applications
#

unexport APP_SUB_DIRS
unexport B_TARG
unexport BI_OBJS
unexport A_TARG
...
_user_o := $(foreach x,$(U_TARG),$($(x)-objs))
_user_o += $(patsubst %,%.o,$(A_TARG))

_usercxx_o := $(foreach x,$(U_CXXTARG),$($(x)-objs))

userspace: $(BI_OBJS) $(BI_XXOBJS) $(_user_o) $(_usercxx_o) \
	$(B_TARG) $(A_TARG) $(L_TARG) $(A_XXTARG) $(B_XXTARG) $(U_TARG) \
	$(U_CXXTARG)

$(B_TARG): % : %.o
	$(CC) $(USER_LDFLAGS) $(EXTRA_USER_LDFLAGS) -o $@ $(patsubst %,%.o,$@) $(USER_LIBS) $(EXTRA_USER_LIBS)

$(BI_OBJS) : %.o : %.c
	$(CC) $(EXTRA_USER_CFLAGS) $(USER_CFLAGS) -c -o $@ $<

$(A_TARG): % : %.o
	$(CC) $(EXTRA_USER_LDFLAGS) $(USER_LDFLAGS) -o $@ $@.o $(USER_LIBS) $(EXTRA_USER_LIBS)

$(B_XXTARG): % : %.o
	$(CC) $(USER_LDFLAGS) $(EXTRA_USER_LDFLAGS) -o $@ $(BI_XXOBJS) $(USER_LIBS) $(EXTRA_USER_LIBS)

$(BI_XXOBJS) : %.o : %.cc
	$(CXX) $(EXTRA_USER_CXXFLAGS) $(USER_CXXFLAGS) -c -o $@ $<

$(A_XXTARG): % : %.cc
	$(CXX) $(EXTRA_USER_CXXFLAGS) $(USER_CXXFLAGS) $(USER_LDFLAGS) $(EXTRA_USER_LDFLAGS) -o $@ $<

$(U_TARG): % : 
	$(CC) $(EXTRA_USER_LDFLAGS) $(USER_LDFLAGS) -o $@ $($@-objs) $(USER_LIBS) $(EXTRA_USER_LIBS) $($@-libs)

$(U_CXXTARG): % : 
	$(CXX) $(EXTRA_USER_LDFLAGS) $(USER_LDFLAGS) -o $@ $($@-objs) $(USER_LIBS) $(EXTRA_USER_LIBS) $($@-libs)

$(_user_o): %.o: %.c
	$(CC) $(EXTRA_USER_CFLAGS) $(USER_CFLAGS) $(CFLAGS_%.o) -c -o $@ $<

$(_usercxx_o): %.o: %.cc
	$(CXX) $(EXTRA_USER_CXXFLAGS) $(USER_CXXFLAGS) $(CXXFLAGS_%.o) -c -o $@ $<

subclean:
ifdef SUBDIRS
	# For some reason, SUBDIRS doesn't actually get unexported
	set -e; for i in $(SUBDIRS); do $(MAKE) -C $$i subclean SUBDIRS= ;done
else
	rm -f *.o core
	rm -f $(A_TARG) $(B_TARG) $(A_XXTARG) $(B_XXTARG) $(U_TARG) $(U_CXXTARG)
ifdef ALL_SUB_DIRS
	set -e; for i in $(ALL_SUB_DIRS); do $(MAKE) -C $$i subclean ;done
endif
endif
