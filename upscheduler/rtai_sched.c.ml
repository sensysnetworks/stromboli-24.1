...
int rt_set_resume_time(RT_TASK *task, RTIME new_resume_time)
{
	...
	if (task->state & DELAYED) {
		...
		if ((task->tnext != &rt_linux_task && (new_resume_time - (task->tnext)->resume_time) > 0) || (new_resume_time - (task->tprev)->resume_time) < 0) {
			rem_timed_task(task);
			enq_timed_task(task);
			rt_schedule();
			hard_restore_flags(flags);
...
int rt_cntsem_wait_if_and_lock(SEM *sem)
{
	...
	if (sem->magic != RT_SEM_MAGIC || sem->type != CNT_SEM) {
		return SEM_ERR;
	}
