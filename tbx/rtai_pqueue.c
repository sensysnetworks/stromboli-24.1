...
// Id:                  @(#)$Id: rtai_pqueue.c,v 1.1.1.1 2004/06/06 14:03:30 rpm Exp $
// Timed extensions (2003): Paolo Mantegazza (mantegazza@aero.polimi.it)
//
...
// ----------------------------------------------------------------------------
static inline BOOL is_empty(struct queue_control *q)
{
...
// ----------------------------------------------------------------------------
static inline BOOL is_full(struct queue_control *q)
{
...
// ----------------------------------------------------------------------------
static BOOL is_blocking(MSG_QUEUE *q)
{
...
// ----------------------------------------------------------------------------
static BOOL can_access(MSG_QUEUE *q, Q_ACCESS access)
{
...
size_t mq_timedreceive(mqd_t mq, char *msg_buffer, size_t buflen, unsigned int *msgprio, const struct timespec *abstime)
{
	...
	while (is_empty(&q->data)) {
		...
		if (is_blocking(q)) {
			DBG("Empty, blocking queue, waiting for a message to arrive\n");
			if (rt_cond_wait_until(&q->emp_cond, &q->mutex, timespec2count(abstime)) < 0) {
				pthread_mutex_unlock(&q->mutex);
				return -ETIMEDOUT;
			}
		} else {
...
int mq_timedsend(mqd_t mq, const char *msg, size_t msglen, unsigned int msgprio, const struct timespec *abstime)
{
	...
	while (is_full(&q->data)) {
		...
		if (is_blocking(q)) {
			DBG("Full, blocking queue %s waiting for space\n", q->q_name);
			if (rt_cond_wait_until(&q->full_cond, &q->mutex, timespec2count(abstime)) < 0) {
				pthread_mutex_unlock(&q->mutex);
				return -ETIMEDOUT;
			}
		} else {
...
#endif
    printk("\n==== RTAI pQueues API v%s (%s, %s) Loaded ====\n", 
