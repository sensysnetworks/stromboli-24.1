...
#define S_FUNCTION_NAME		ni_pci_di
...
static void mdlStart(SimStruct *S)
{
	...
	fprintf(logfile, "Board id    : 0x%x\n", dev_id);
        ...
        if (DIO_82C55) {
                ...
                fprintf(logfile, "Number of DI lines activated : %d\n", NUM_CHANS_82C55);
                ni_82C55_configure(port_82C55, NI_82C55_INPUT, Board);
        }
        if (DIO_MODULE) {
                fprintf(logfile, "Digital I/O module\n");
                fprintf(logfile, "DI line activated : %d\n", DIO_MODULE_LINE - 1);
                ni_DIO_module_configure((int)(DIO_MODULE_LINE - 1), NI_DIO_MODULE_INPUT, Board);
        }
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	int n;
	unsigned char DI_byte_DIO_module = 0;
	unsigned char DI_byte_82C55 = 0;

#ifndef MATLAB_MEX_FILE

	if (DIO_82C55) {
		ni_82C55_read_byte(&DI_byte_82C55, port_82C55, Board);
		for (n = 0; n < NUM_CHANS_82C55; n++) {
			*y++ = (double)((DI_byte_82C55 & (1 << n)) >> n);
		}
	}
	if (DIO_MODULE) {
		ni_DIO_module_read_bit(&DI_byte_DIO_module, DIO_module_line, Board);
		*y++ = (double)(DI_byte_DIO_module >> DIO_module_line);
	}
