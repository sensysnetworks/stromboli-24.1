...
$Id: proxies.c,v 1.1.1.1 2004/06/06 14:02:34 rpm Exp $ 
*/
...
RT_TASK *__rt_proxy_attach(void (*agent)(int), RT_TASK *task, void *msg, int nbytes, int priority)
{
	...
	if (rt_task_init(proxy, agent, (int)proxy, PROXY_MIN_STACK_SIZE + nbytes + sizeof(struct proxy_t), priority, 0, 0)) {
		rt_free(proxy);
		return 0;
	}
