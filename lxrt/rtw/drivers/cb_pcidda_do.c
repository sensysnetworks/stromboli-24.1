...
#define S_FUNCTION_NAME		cb_pcidda_do
...
#define THRESHOLD_PARAM		ssGetSFcnParam(S,3)
...
static void mdlCheckParameters(SimStruct *S)
{
	...
	if (THRESHOLD < 0) {
		sprintf(errMsg, "Threshold must be a positive value.\n");
		allParamsOK = 0;
		goto EXIT_POINT;
	}
	if (SAMPLE_TIME <= 0) {
...
static void mdlStart(SimStruct *S)
{
	...
	printf("Port        : %c (OUTPUT)\n", letter[port-1]);
	printf("N channels  : %d\n", nchan);
	cb_pcidda_configure_dio(number, port, D_OUT);
#endif
}
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	cb_pcidda_write_do(number, port, DO_byte);
#endif
}
