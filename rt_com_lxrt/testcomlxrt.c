...
int main(int argc, char **argv)
{
	...
	// this example use rt_com_setup() on port PORT so it needs that
	// you have compiled rt_com.c with rt_com_table[PORT].used=1
	// otherwise rt_com_setup() fails
	
	if ( rt_com_setup(PORT, 9600, RT_COM_NO_HAND_SHAKE, RT_COM_PARITY_NONE, 1, 8, -1) < 0) {
		printf("testcomlxrt: rt_com_setup() error\n");
		retval = 1;
	} else {	
		rt_com_write(PORT, "Begin serial Test\n\r", 20);
		printf("<1> RT_Com_LXRT Test Program - Start\n");
		...
		while (!end) {
	    	...
	    	rt_task_wait_period();
       	    ...
       	    n = rt_com_read(PORT, buf+sl,5); 
       
       	    if (n > 0) {
				strcpy(buf+sl+n,"\r\n");
				rt_com_write(PORT, buf, strlen(buf));
	    	}
		}
		rt_com_setup(PORT, -1, 0, 0, 0, 0, 0);  // release port
	}

	rt_make_soft_real_time();
	stop_rt_timer();
	rt_task_delete(testcomtsk);
	printf ("<1> RT_Com_LXRT Test Program - End\n");
	exit(retval);
}
