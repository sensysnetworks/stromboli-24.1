...
// Id:                  @(#)$Id: simple_q1.c,v 1.1.1.1 2004/06/06 14:03:04 rpm Exp $
//
...
static const char str2[27] = "This is test string num 2!\0";
  ...
  if (my_q > 0) {
    ...
    //Put 2 messages on the queue
    //n = mq_send(my_q, "Test string number one", 22, 1); 
    n = mq_send(my_q, str1, 22, 1); 
    rt_printk("Parent put message 1 of %d bytes on queue at priority %d\n",
					n, 1);
    n = mq_send(my_q, str2, 27, 5); 
    rt_printk("Parent put message 2 of %d bytes on queue at priority %d\n",
					n, 5);
