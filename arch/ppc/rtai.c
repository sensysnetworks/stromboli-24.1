...
void __rt_mount_rtai(void)
{
	 ...
	 */
#if 0
#define DBGMSR(fmt,args...)    printk(fmt ,##args)
#else
#define DBGMSR(fmt,args...)
#endif
#define MSR(x) ((struct pt_regs *)((x)->thread.ksp + STACK_FRAME_OVERHEAD))->msr
	task = &init_task;
	if ((MSR(task) & MSR_EE) == 0) {
		MSR(task) |= MSR_EE;
		DBGMSR("Fix MSR to 0x%04lx in stack of '%s' (0x%p)\n", 
		       MSR(task), task->comm, task);
	}
	if (task->thread.regs) {
		if (((task->thread.regs)->msr & MSR_EE) == 0) {
			(task->thread.regs)->msr |= MSR_EE;
			DBGMSR("Fix MSR to 0x%04lx in regs of  '%s' (0x%p)\n", 
			       (task->thread.regs)->msr, task->comm, task);
		}
	}
	for_each_task(task) {
		if (task == current)
